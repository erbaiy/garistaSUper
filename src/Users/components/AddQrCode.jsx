...
function AddQrCode() {
  ...
  const fetchUsers = () => {
    setLoading(true);
    fetch("http://127.0.0.1:8000/api/restaurants-owners")
  ...
  const handleAddUser = async () => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/restaurants-owners",
        {
          method: "POST",
          headers: {
  ...
  //       // You can add code here to close the modal
  //     }
  //   } catch (error) {
  //     console.error("Error inserting data:", error);
  //     setErrors({ general: "Failed to add user" });
  ...
  const handleConnectClick = async (user) => {
    try {
      const response = await apiService.post(`/connect/${user.id}`);
      if (response.status === 200) {
        // Handle success, show success message
        toast.success(`Connected as ${user.first_name}`);
        alert(user);
        // Redirect to dashboard
        window.location.href = "/dashboard"; // Directly changing window location
      } else {
            ...
            <DropdownMenu className="flex justify-end">
              ...
              <DropdownMenuContent className="w-56">
                ...
                >
                  ...
                  <DropdownMenuItem onSelect={() => handleConnectClick(user)}>
                    Connect
                  </DropdownMenuItem>
                </DropdownMenuRadioGroup>
              </DropdownMenuContent>
